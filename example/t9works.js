...
async function test() {
  ...
  if (tokenRet.code != ErrorCode.SUCCESS) {
    ...
    for (let variable in variables) {
      if (variable in tokenRet.result['0x71be2bb1a994447420915c4534cbf5d0305480a6']) {
        if (variable === 'buy_tax' || variable === 'sell_tax') {
          if (Number(tokenRet.result['0x71be2bb1a994447420915c4534cbf5d0305480a6'][variable]) > 5) {
            isGem = false;
            break;
          }
        } else {
          if (tokenRet.result['0x71be2bb1a994447420915c4534cbf5d0305480a6'][variable] !== variables[variable]) {
            isGem = false;
            break;
          }
        }
